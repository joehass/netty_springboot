...
/**
 * 例如：<iq type='set' id='_bind_auth_2' xmlns='jabber:client'><bind xmlns='urn:ietf:params:xml:ns:xmpp-bind'><resource>pc</resource></bind></iq>
...
public final class XMLElementImpl implements XMLElement {
	...
	public ImmutableMap<String, String> getAttributes() {
		final ImmutableMap.Builder<String, String> result = ImmutableMap
				.builder();
	...
	public XMLElement addChild(final String name,
		...
		if (namespace != null) {
			newElement = document
					.createElementNS(namespace, checkNotNull(name));
		} else {
	...
	public XMLElement addChild(final HasXML child) {
		...
		final Element newElement = (Element) document.importNode(
				((XMLElementImpl) child.getXML()).element, true);
		element.appendChild(newElement);
		return new XMLElementImpl(newElement);
	}
	...
	public ImmutableList<XMLElement> getChildren(final String name,
		...
		final ImmutableList.Builder<XMLElement> result = ImmutableList
				.builder();
	...
	@Override
	public ImmutableList<XMLElement> getChildren(
			final Predicate<XMLElement> matcher) {
		checkNotNull(matcher);
	...
	@Override
	public void setChildText(final String name,
                             @Nullable final String namespace, @Nullable final String text) {
	 ...
	 */
	public final static XMLElement fromString(final String xml) {
	 ...
	 * @return the resulting element
	 */
	public final static XMLElement fromElement(final Element element) {
		return new XMLElementImpl(element);
	}
